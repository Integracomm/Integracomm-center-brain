...
HELP: dict[str, list[tuple[str, str]]] = {
    ...
    "growth/carga": [
        ("_intro", "O risco da carteira visto por TIME, não por conta: qual squad concentra contas críticas, MRR em risco e execução atrasada — e se o tamanho do time dá conta da carteira que atende. Objetivo: decidir realocação/reforço ANTES de a sobrecarga virar churn. Tom de alocação e suporte, não ranking de culpados. Squads = os reais da planilha de composição (Bx-Sy); o líder de cada squad é o Growth."),
        ("Por squad", "Carga de risco de cada squad: contas, MRR sob gestão, alertas abertos por severidade, MRR em risco (com % do total) e execução atrasada; as bolinhas mostram a distribuição de faixas de saúde. Chip 'concentração de risco' = ≥3 críticos ou ≥30% do MRR em risco num squad só. Insights: (1) squad com chip = pauta da semana da coordenação; (2) muitos críticos com MRR baixo × poucos críticos com MRR alto pedem respostas diferentes — o segundo caso é reforço imediato."),
