...
HELP: dict[str, list[tuple[str, str]]] = {
    ...
    "vendas/horarios": [
        ("_intro", "Existe hora melhor para a REUNIÃO acontecer? Base: momento em que o deal entrou em Negociação (o card é movido após a reunião — proxy do horário dela, em Brasília); 'fechou' = deal ganho a qualquer tempo. Mapa de calor dia×hora + conversão por hora. Ressalva: closer que atualiza cards em lote infla certos horários — a coordenação sabe distinguir."),
        ("Mapa de calor", "Células escuras = mais reuniões naquele dia/hora; o tooltip traz a conversão da célula. Insight: volume e conversão são perguntas diferentes — a janela cheia não é necessariamente a que fecha mais."),
        ("Conversão por hora da reunião", "Reuniões, fechamentos e taxa por hora do dia. Insights: (1) se uma janela converte consistentemente mais (com amostra razoável), priorize-a ao agendar — é argumento para a Pré-vendas oferecer esses horários primeiro; (2) taxas parecidas em todas as horas = o horário não é variável relevante, não vale reorganizar agenda por isso."),
    ],
    "vendas/forecast": [
