...
HELP: dict[str, list[tuple[str, str]]] = {
    ...
    # ============================= PRÉ-VENDAS ===========================
    "prevendas/funil": [
        ("_intro", "Funil da qualificação com a MESMA taxonomia e régua do Funil de Prospecção do Marketing (validada contra o Pipedrive): Lead → MQL → SAL → SQL, contagem por evento no período, corte de Brasília — os números batem com a aba Funil do Marketing e com o dashboard do time. Pré-vendas termina no SQL (reunião agendada = handoff para Vendas); Oportunidade e Booking seguem no Funil de Fechamento. Responde: os leads estão virando reunião? Onde morrem os que não viram?"),
        ("Funil", "Lead (criados no período) → MQL (perfil validado — mesma contagem do Lead) → SAL (recebeu o 1º contato) → SQL (reunião agendada, handoff para Vendas), com taxas vs etapa anterior e vs Lead. Insights: (1) queda forte em MQL→SAL = problema de velocidade/fila (veja Speed-to-Lead); (2) queda em SAL→SQL = lead ruim ou abordagem — cruze com a qualidade por origem."),
