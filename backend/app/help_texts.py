...
HELP: dict[str, list[tuple[str, str]]] = {
    ...
    "financeiro/visao": [
        ("_intro", "Planejamento × realizado da planilha Planejamento_Receita_2026 + tempo real do Pipedrive. Três blocos: mês corrente ao vivo vs meta (ritmo do mês), histórico fechado e metas dos próximos meses (por bundle + funil projetado). Recebimento/inadimplência não têm fonte em tempo real até os dados do Omie entrarem — acompanhe no histórico mensal (validação 15/07: bookings e taxas do Pipedrive bateram ao centavo com a conferência manual do gestor)."),
        ("Mês em tempo real × meta", "Funil e bookings ao vivo do espelho do Pipedrive (mesma régua oficial do funil de Marketing, defasagem ≤10 min). O verde/vermelho compara o % da meta atingido com a fração do mês decorrida (ritmo): verde = no ritmo ou acima. Insights: (1) receita no ritmo mas bookings abaixo = ticket médio acima do plano — checar mix por bundle; (2) funil no ritmo mas bookings atrás = gargalo na ponta de fechamento, ver Vendas."),
        ("Histórico realizado", "Meses fechados direto da planilha: recebimento, % recorrente, inadimplência, bookings vs meta e churn. Insight: % recorrente subindo + inadimplência caindo = qualidade de receita melhorando, mesmo que o total oscile."),
        ("Metas — mês corrente e próximos", "O plano dos próximos meses por bundle (qtde, R$, ticket) e o funil projetado com as taxas-alvo. Insight: a meta de leads cresce mês a mês — se a taxa Oportunidade→Booking planejada (15%) não se confirmar no realizado, o plano de bookings fica em risco antes do fim do trimestre."),
        ("Recebimento mês a mês", "Barra clara = recebimento total; barra escura dentro = a parte recorrente (% sob o mês); tracejado = projeção da planilha. Insights: (1) a barra escura crescendo mais rápido que a clara = qualidade de receita melhorando; (2) queda no total com recorrente estável = perda pontual de não-recorrente, menos grave que perda de base."),
        ("Bookings × meta mês a mês", "Barra verde = meta do mês batida; vermelha = abaixo; o tique tracejado marca a meta; meses futuros mostram a meta vazada. O nº de bookings aparece sob cada mês. Insight: sequência de vermelhos com qtde em dia = problema de ticket/mix (B1 demais), não de volume."),
        ("Inadimplência e churn", "Chips mensais coloridos por limiar (inadimplência: verde ≤4%, amarelo ≤8% · churn: verde ≤5%, amarelo ≤9%); meses futuros = alvo, esmaecidos. Insight: inadimplência e churn subindo JUNTOS costuma indicar problema de cobrança/valor percebido no mesmo grupo de contas — cruzar com Cancelamentos do Growth."),
    ],
    "financeiro/receita": [
        ("_intro", "Saúde da Receita Recorrente (mudou da Visão central p/ cá em 15/07). ISR = base recorrente ÷ mês anterior ×100 (≥100 = crescendo); Quick Ratio = nova ÷ perdida (≥1 = repõe o churn). Duas visões que NÃO se misturam: B2-B5 = o sinal do modelo novo; Consolidado = caixa com os planos antigos em runoff. ★ = crossover projetado (mês em que o B2-B5 sustenta a receita sozinho)."),
    ],
    "growth/carga": [
