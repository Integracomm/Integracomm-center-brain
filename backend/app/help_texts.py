...
HELP: dict[str, list[tuple[str, str]]] = {
    ...
    "prevendas/sdrs": [
        ("_intro", "Duas réguas complementares por pessoa + planos de ação por regra. A OFICIAL (primeira tabela) é a dos gráficos do Pipedrive: atribuição pelo campo SDR do deal — é a que bate com o Insights. A OPERACIONAL (segunda) atribui por quem fez o 1º contato registrado — mede esforço e velocidade. A lista do time é editável no Painel Administrativo."),
        ("Leads e oportunidades por colaborador", "Régua oficial: leads = deals criados no período; oportunidades = campo Dia Oportunidade no período (não é coorte — a oportunidade pode ser de lead antigo); bookings = ganhos no período; atribuição = campo SDR do deal, sem fallback. Insights: (1) estes números conferem com os gráficos do Pipedrive; (2) '(sem SDR definido)' grande = leads ainda não distribuídos/tocados — é a fila a zerar (cruze com Speed-to-Lead); (3) Lead→Oport muito diferente entre SDRs com volume parecido = mix de origem ou abordagem — cruze com a régua operacional; (4) nomes 'fora do time de PV' têm deals no nome (closers, ex-SDRs) — ajuste a lista no Painel Administrativo se alguém entrou/saiu."),
        ("Produtividade por SDR", "Régua operacional: leads TOCADOS (1º contato registrado), reuniões geradas, taxa e speed mediano. Insights: (1) compare com a MEDIANA do time, não entre extremos; (2) taxa alta com volume baixo = capacidade ociosa; volume alto com taxa baixa = qualidade de abordagem ou de lead — o speed ajuda a separar."),
